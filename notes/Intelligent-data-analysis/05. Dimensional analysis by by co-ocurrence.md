...
> *Adapted and expanded from the lecture slides by Felipe Orihuela‑Espina (28 Oct 2024).*  
> This read‑me mirrors the original slide structure but adds plain‑English commentary, worked examples, and gentle algebra refreshers. Everything is formatted for GitHub Markdown with `$…$` math and matrices rendered as tables.

---

# Section&nbsp;1 — Latent Semantic Analysis (Problem Statement)

## 1.1  LSA credentials
Latent Semantic Analysis (LSA) surfaced in 1990 (Deerwester _et al._) as a technique for **discovering latent concepts hidden in a document‑term co‑occurrence matrix**. At its core, LSA is just a *Singular Value Decomposition* (SVD) applied to the count matrix, so it travels well beyond text — any tall, rectangular data set with meaningful co‑occurrences can ride the same math.
...
1. **PCA recap**:  
   *Data matrix* $\mathbf{X} \in \mathbb{R}^{m \times n}$ (rows = observations, columns = variables)  
   Covariance $\mathbf{A}=\operatorname{cov}(\mathbf{X})$ is $n\times n$, symmetric, and amenable to eigendecomposition.
...
1. Deerwester, S. _et al._ “Indexing by Latent Semantic Analysis.” *JASIS* (1990)  
2. Landauer, T. & Dumais, S. “A Solution to Plato’s Problem.” *Psychological Review* (1997)

---

© 2025-04-27

# Section 2 — Complex Conjugates & the Conjugate Transpose Matrix

> *Expanded notes based on Felipe Orihuela‑Espina’s slide deck (pp. 18‑23, 28 Oct 2024).*  
> Plain‑English explanations, $…$ math, and GitHub‑friendly matrix tables included.

---

## 2.1  Complex Conjugate — definition & example

A complex number $z = a + b i$ has *conjugate* $z^{\*} = a - b i$. citeturn8file2
...
## 2.2  Key algebraic properties citeturn8file2
...
## 2.3  Why do we care? citeturn8file0
...
## 2.4  Conjugate Transpose (Hermitian Transpose) citeturn8file2
...
*(Table format keeps GitHub’s markdown renderer happy.)*

---

## 2.5  Unitary Matrices citeturn8file0
...
# Section 3 — Matrix Inversion

> *Expanded notes based on Felipe Orihuela‑Espina’s slide deck (pp. 24‑47, 28 Oct 2024).*  
> We follow the original flow but add intuitive commentary, fully worked examples, and GitHub‑friendly tables. citeturn9file0
...
## 3.2  Existence & Uniqueness Theorems citeturn9file0
...
### 3.3.1  Cofactors & Cofactor Matrix citeturn9file0
...
Collect these into the **cofactor matrix** $\operatorname{cof}(\mathbf{A})$; transpose to get the **adjoint** $\operatorname{adj}(\mathbf{A})$.

### 3.3.2  Key identity  
$$\mathbf{A}\,\operatorname{adj}(\mathbf{A}) = \operatorname{adj}(\mathbf{A})\,\mathbf{A} = \det(\mathbf{A})\mathbf{I}$$

### 3.3.3  Inverse formula  
$$\boxed{\;\mathbf{A}^{-1} = \dfrac{\operatorname{adj}(\mathbf{A})}{\det(\mathbf{A})}\;}$$
...
2. **Cofactor matrix** $\operatorname{cof}(\mathbf{A})$ (already sign‑corrected):
...
1. Confirm $\det(\mathbf{A})\neq0$.  
2. Pick your algorithm: adjoint (tiny $n$), Gauss‑Jordan (general), or specialised block/partition methods.  
3. Remember: *invert only when you must* — solve $\mathbf{A}\mathbf{x}=\mathbf{b}$ with decompositions (LU, QR) instead.
...
> *Expanded notes based on Felipe Orihuela‑Espina’s slide deck (pp. 48‑59, 28 Oct 2024).*  
> Think of SVD as “eigendecomposition for everyone”—it works on any rectangular matrix and spits out orthogonal bases plus a power‑ranked diagonal of *singular values*. citeturn10file0
...
When $\mathbf{A}$ is real, $\mathbf{V}^{\*}$ simplifies to $\mathbf{V}^{\top}$. citeturn10file0
...
Net effect: stretch and re‑orient, just like eigendecomposition, but without the square‑matrix handcuffs. citeturn10file0

---

## 4.3  Why the conjugate transpose? citeturn10file0
...
*(Real code lives in LAPACK’s `dgesdd`.)*
...
## 4.8  Cheat‑Sheet Summary

1. **Decompose:** $\mathbf{A} = \mathbf{U}\boldsymbol{\Sigma}\mathbf{V}^{\*}$ always.  
2. **Orthogonal magic:** columns of $\mathbf{U}$ and $\mathbf{V}$ are perpendicular.  
3. **Singular values:** non‑negative scalings in descending order, squared they’re eigenvalues.  
4. **Compression:** truncate $\sigma$’s to denoise or reduce dimensionality.  
5. **Software:** call `numpy.linalg.svd` or your BLAS flavour—do *not* roll your own in production.

---

© 2025-04-27

# Section 5 — Latent Semantic Analysis: *Putting It All Together*

> *Expanded notes based on Felipe Orihuela‑Espina’s slide deck (pp. 60‑64, 28 Oct 2024).*  
> Here we connect the mathematical plumbing (Sections 1‑4) to the final LSA workflow and its real‑world pay‑offs. citeturn11file0
...
LSA demands a **two‑mode** analysis (terms × documents) — a rectangular matrix. Enter SVD. citeturn11file0
...
* Two‑mode problem ⇒ **SVD**.  
* Keep top $k$ singulars ⇒ noise reduction + semantic compression.  
* Latent vectors ≠ interpretable topics, yet excel at similarity tasks.  
* LSA laid the groundwork for modern embeddings (word2vec, BERT).
